...
class ValidateMandatoryBookingPropertyTest {
    ...
    void testMissingBookingMethodOnLineAndJourneyPatternAndServiceJourney() {
        ValidateMandatoryBookingProperty validateMandatoryBookingProperty = new ValidateMandatoryBookingProperty("BookingMethods", "frames/");
        ...
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateMandatoryBookingProperty.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
        Assertions.assertFalse(xPathValidationReportEntries.isEmpty());
        Assertions.assertEquals(validateMandatoryBookingProperty.getCode(), xPathValidationReportEntries.get(0).getCode());
    }
    ...
    void testBookingMethodOnLine() {
        ValidateMandatoryBookingProperty validateMandatoryBookingProperty = new ValidateMandatoryBookingProperty("BookingMethods", "frames/");
        String flexibleLineWithInvalidBookingAccess = NETEX_FRAGMENT
                .replace("${BOOKING_METHOD}", "<BookingMethods>callOffice online</BookingMethods>")
                .replace("${BOOKING_ARRANGEMENT}", "")
                .replace("${FLEXIBLE_PROPERTIES}", "");
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(flexibleLineWithInvalidBookingAccess);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
    ...
    void testBookingMethodInJourneyPattern() {
        ...
        String journeyPatternWithBookingArrangement = NETEX_FRAGMENT
                .replace("${BOOK_WHEN}", "")
                .replace("${BOOKING_ARRANGEMENT}", "<BookingArrangements><BookingMethods>callOffice online</BookingMethods></BookingArrangements>")
                .replace("${FLEXIBLE_PROPERTIES}", "");
    ...
    void testBookingMethodInServiceJourney() {
        ...
        String serviceJourneyWithFlexibleProperties = NETEX_FRAGMENT
                .replace("${BOOK_WHEN}", "")
                .replace("${BOOKING_ARRANGEMENT}", "")
                .replace("${FLEXIBLE_PROPERTIES}", "<FlexibleServiceProperties><BookingMethods>callOffice online</BookingMethods></FlexibleServiceProperties>");
