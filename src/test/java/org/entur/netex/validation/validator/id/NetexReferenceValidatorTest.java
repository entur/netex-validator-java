...
class NetexReferenceValidatorTest {
    ...
    void setUpTest() {
        ...
        netexIdRepository = new NetexIdRepository() {
            @Override
    ...
    void testUnresolvedExternalReferencesToLocalIds() {
        ...
        IdVersion localRef = new IdVersion(TEST_MISSING_EXTERNAL_ID, null, "YY", null, null, 0, 0);
        List<IdVersion> localRefs = List.of(localRef);
        ValidationContext validationContext = new ValidationContext(null, null, TEST_CODESPACE, null, localIds, localRefs);
        NetexReferenceValidator netexReferenceValidator = new NetexReferenceValidator(netexIdRepository, List.of(), validationReportEntryFactory);
        ...
        Assertions.assertEquals(NetexReferenceValidator.RULE_CODE_NETEX_ID_5, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }
    ...
    void testUnresolvedExternalReferencesToExternalRefs() {
        IdVersion localRefVersion = new IdVersion(TEST_LOCAL_ID, null, "YY", null, null, 0, 0);
        Set<IdVersion> localIds = Set.of();
        List<IdVersion> localRefs = List.of(localRefVersion);
        ValidationContext validationContext = new ValidationContext(null, null, TEST_CODESPACE, null, localIds, localRefs);

        ExternalReferenceValidator externalReferenceValidator = externalIdsToValidate -> Set.of();
        NetexReferenceValidator netexReferenceValidator = new NetexReferenceValidator(netexIdRepository, List.of(externalReferenceValidator), validationReportEntryFactory);
        netexReferenceValidator.validate(validationReport, validationContext);
        Assertions.assertFalse(validationReport.getValidationReportEntries().isEmpty());
        Assertions.assertEquals(NetexReferenceValidator.RULE_CODE_NETEX_ID_5, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }
    ...
    void testNoUnresolvedExternalReferencesToExternalRefs() {
        ...
        ExternalReferenceValidator externalReferenceValidator = externalIdsToValidate -> Set.of(localRefVersion);
        NetexReferenceValidator netexReferenceValidator = new NetexReferenceValidator(netexIdRepository, List.of(externalReferenceValidator), validationReportEntryFactory);
        netexReferenceValidator.validate(validationReport, validationContext);
        Assertions.assertTrue(validationReport.getValidationReportEntries().isEmpty());
    }
