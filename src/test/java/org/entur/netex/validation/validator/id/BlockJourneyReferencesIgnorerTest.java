...
class BlockJourneyReferencesIgnorerTest {
    ...
    void testFilterAllReferencesNotOriginatingFromBlock() {
        IdVersion idVersion = new IdVersion(TEST_DEAD_RUN_REFERENCE_ID, null, "DeadRunRef", List.of("Not a Block"), null, 0, 0);
        Set<IdVersion> externalIdsToValidate = Set.of(idVersion);
        Set<IdVersion> filteredIdVersions = blockJourneyReferencesIgnorer.validateReferenceIds(externalIdsToValidate);
    ...
    void testFilterAllReferencesOriginatingFromBlockAndNotReferringToServiceJourneyOrDeadRun() {
        IdVersion idVersion = new IdVersion(TEST_NON_JOURNEY_REFERENCE_ID, null, "YYRef", List.of("Block"), null, 0, 0);
        Set<IdVersion> externalIdsToValidate = Set.of(idVersion);
        Set<IdVersion> filteredIdVersions = blockJourneyReferencesIgnorer.validateReferenceIds(externalIdsToValidate);
        Assertions.assertNotNull(filteredIdVersions);
        Assertions.assertTrue(filteredIdVersions.isEmpty());
    ...
    void testAcceptReferenceToServiceJourneyFromBlock() {
        IdVersion idVersion = new IdVersion(TEST_SERVICE_JOURNEY_REFERENCE_ID, null, "ServiceJourneyRef", List.of("Block"), null, 0, 0);
        Set<IdVersion> externalIdsToValidate = Set.of(idVersion);
        Set<IdVersion> filteredIdVersions = blockJourneyReferencesIgnorer.validateReferenceIds(externalIdsToValidate);
        Assertions.assertNotNull(filteredIdVersions);
    ...
    void testAcceptReferenceToDeadRunFromBlock() {
        IdVersion idVersion = new IdVersion(TEST_DEAD_RUN_REFERENCE_ID, null, "DeadRunRef", List.of("Block"), null, 0, 0);
        Set<IdVersion> externalIdsToValidate = Set.of(idVersion);
        Set<IdVersion> filteredIdVersions = blockJourneyReferencesIgnorer.validateReferenceIds(externalIdsToValidate);
        Assertions.assertNotNull(filteredIdVersions);
        Assertions.assertEquals(externalIdsToValidate, filteredIdVersions);
