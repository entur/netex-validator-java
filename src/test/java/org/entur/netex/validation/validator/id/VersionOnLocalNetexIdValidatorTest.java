...
class VersionOnLocalNetexIdValidatorTest {
    ...
    void setUpTest() {
        ...
        versionOnLocalNetexIdValidator = new VersionOnLocalNetexIdValidator(validationReportEntryFactory);
    ...
    void testLocalReferenceWithoutVersion() {
        ...
        versionOnLocalNetexIdValidator.validate(validationReport, validationContext);
        Assertions.assertFalse(validationReport.getValidationReportEntries().isEmpty());
        Assertions.assertEquals(VersionOnLocalNetexIdValidator.RULE_CODE_NETEX_ID_8, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }
