...
class ValidateMandatoryBookingWhenOrMinimumBookingPeriodPropertyTest {
    ...
    private static final String NETEX_FRAGMENT = "                <frames xmlns=\"http://www.netex.org.uk/netex\">" +
            "<ServiceFrame>\n" +
            "                     <lines>\n" +
    ...
    void testBookWhenMissingInLineAndStopPointAndServiceJourney() {
        ValidateMandatoryBookingWhenOrMinimumBookingPeriodProperty validateMandatoryBookingWhenOrMinimumBookingPeriodProperty = new ValidateMandatoryBookingWhenOrMinimumBookingPeriodProperty("frames/");
        String flexibleLineWithMissingBookWhen = NETEX_FRAGMENT
                .replace("${BOOK_WHEN}", "")
                .replace("${BOOKING_ARRANGEMENT}", "")
                .replace("${FLEXIBLE_PROPERTIES}", "");
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(flexibleLineWithMissingBookWhen);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateMandatoryBookingWhenOrMinimumBookingPeriodProperty.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
        Assertions.assertFalse(xPathValidationReportEntries.isEmpty());
        Assertions.assertEquals(validateMandatoryBookingWhenOrMinimumBookingPeriodProperty.getCode(), xPathValidationReportEntries.get(0).getCode());
    }
    ...
    void testBookWhenInLine() {
        ...
        String flexibleLineWithBookWhen = NETEX_FRAGMENT
                ...
                .replace("${BOOKING_ARRANGEMENT}", "")
                .replace("${FLEXIBLE_PROPERTIES}", "");
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(flexibleLineWithBookWhen);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateMandatoryBookingWhenOrMinimumBookingPeriodProperty.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
    ...
    void testBookWhenInJourneyPattern() {
        ...
        String journeyPatternWithBookingArrangement = NETEX_FRAGMENT
                .replace("${BOOK_WHEN}", "")
                .replace("${BOOKING_ARRANGEMENT}", "<BookingArrangements><BookWhen>advanceAndDayOfTravel</BookWhen></BookingArrangements>")
                .replace("${FLEXIBLE_PROPERTIES}", "");
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(journeyPatternWithBookingArrangement);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateMandatoryBookingWhenOrMinimumBookingPeriodProperty.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
        Assertions.assertTrue(xPathValidationReportEntries.isEmpty());
    ...
    void testBookWhenInServiceJourney() {
        ...
        String serviceJourneyWithFlexibleProperties = NETEX_FRAGMENT
                .replace("${BOOK_WHEN}", "")
                .replace("${BOOKING_ARRANGEMENT}", "")
                .replace("${FLEXIBLE_PROPERTIES}", "<FlexibleServiceProperties><BookWhen>advanceAndDayOfTravel</BookWhen></FlexibleServiceProperties>");
        XdmNode document = NETEX_XML_PARSER.parseStringToXdmNode(serviceJourneyWithFlexibleProperties);
        XPathValidationContext xpathValidationContext = new XPathValidationContext(document, NETEX_XML_PARSER, TEST_CODESPACE, null);
        List<XPathValidationReportEntry> xPathValidationReportEntries = validateMandatoryBookingWhenOrMinimumBookingPeriodProperty.validate(xpathValidationContext);
        Assertions.assertNotNull(xPathValidationReportEntries);
        Assertions.assertTrue(xPathValidationReportEntries.isEmpty());
