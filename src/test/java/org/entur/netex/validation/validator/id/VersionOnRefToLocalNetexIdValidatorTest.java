...
class VersionOnRefToLocalNetexIdValidatorTest {
    ...
    void setUpTest() {
        ...
        versionOnRefToLocalNetexIdValidator = new VersionOnRefToLocalNetexIdValidator(validationReportEntryFactory);
        validationReport = new ValidationReport(TEST_CODESPACE, TEST_VALIDATION_REPORT_ID);
    }

    @Test
    void testLocalReferenceWithoutVersion() {
        IdVersion idVersionLocalRef = new IdVersion(TEST_REFERENCED_ID, null, "YYRef", null, null, 0, 0);
        List<IdVersion> localRefs = List.of(idVersionLocalRef);
        IdVersion idVersionLocal = new IdVersion(TEST_REFERENCED_ID, null, "YY", null, null, 0, 0);
        ...
        versionOnRefToLocalNetexIdValidator.validate(validationReport, validationContext);
        Assertions.assertFalse(validationReport.getValidationReportEntries().isEmpty());
        Assertions.assertEquals(VersionOnRefToLocalNetexIdValidator.RULE_CODE_NETEX_ID_9, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }

    @Test
    void testLocalReferenceWithVersion() {
        IdVersion idVersionLocalRef = new IdVersion(TEST_REFERENCED_ID, "1" , "YYRef", null, null, 0, 0);
        List<IdVersion> localRefs = List.of(idVersionLocalRef);
        IdVersion idVersionLocal = new IdVersion(TEST_REFERENCED_ID, "1", "YY", null, null, 0, 0);
        Set<IdVersion> localIds = Set.of(idVersionLocal);
        ValidationContext validationContext = new ValidationContext(null, null, TEST_CODESPACE, null, localIds, localRefs);
        versionOnRefToLocalNetexIdValidator.validate(validationReport, validationContext);
