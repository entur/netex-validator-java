...
class NetexIdUniquenessValidatorTest {
    ...
    void setUpTest() {
        ...
        netexIdUniquenessValidator = new NetexIdUniquenessValidator(netexIdRepository, validationReportEntryFactory);
    ...
    void testDuplicatedIdInLineFile() {
        ...
        Assertions.assertEquals(NetexIdUniquenessValidator.RULE_CODE_NETEX_ID_1, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }
    ...
    void testDuplicatedIdInCommonFile() {
        IdVersion idVersion = new IdVersion(TEST_DUPLICATED_ID, null, "YY", null, null, 0, 0);
        Set<IdVersion> localIds = Set.of(idVersion);
        ValidationContext validationContext = new ValidationContext(null, null, TEST_CODESPACE, "_common.xml", localIds, List.of());
        netexIdUniquenessValidator.validate(validationReport, validationContext);
        Assertions.assertFalse(validationReport.getValidationReportEntries().isEmpty());
        Assertions.assertEquals(NetexIdUniquenessValidator.RULE_CODE_NETEX_ID_10, validationReport.getValidationReportEntries().stream().findFirst().orElseThrow().getName());
    }

    @Test
    void testNoDuplicateId() {
        IdVersion idVersion = new IdVersion(TEST_NOT_DUPLICATED_ID, null, "YY", null, null, 0, 0);
        Set<IdVersion> localIds = Set.of(idVersion);
        ValidationContext validationContext = new ValidationContext(null, null, TEST_CODESPACE, null, localIds, List.of());
        netexIdUniquenessValidator.validate(validationReport, validationContext);
