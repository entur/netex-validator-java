...
public class ValidateMandatoryBookingWhenOrMinimumBookingPeriodProperty extends AbstractXPathValidationRule {
    ...
    public ValidateMandatoryBookingWhenOrMinimumBookingPeriodProperty(String context) {
    ...
    public List<XPathValidationReportEntry> validate(XPathValidationContext validationContext) {
        try {
            ...
            XPathSelector missingFieldSelector = validationContext.getNetexXMLParser().getXPathCompiler().compile(context + "ServiceFrame/lines/FlexibleLine and " + context + "ServiceFrame/lines/FlexibleLine[not(BookWhen) and not(MinimumBookingPeriod)]").load();
            ...
            if (missingField) {
                XPathSelector selector = validationContext.getNetexXMLParser().getXPathCompiler().compile(context + "ServiceFrame/journeyPatterns/*[self::JourneyPattern][pointsInSequence/StopPointInJourneyPattern[not(BookingArrangements/BookWhen) and not(BookingArrangements/MinimumBookingPeriod)  ]]").load();
                ...
                for (XdmItem item : nodes) {
                    if (item instanceof XdmNode) {
                        ...
                        XPathSelector sjSelector = validationContext.getNetexXMLParser().getXPathCompiler().compile(context + "TimetableFrame/vehicleJourneys/ServiceJourney[(not(FlexibleServiceProperties) or (not(FlexibleServiceProperties/BookWhen) and not(FlexibleServiceProperties/MinimumBookingPeriod) )) and JourneyPatternRef/@ref='" + id + "' and JourneyPatternRef/@version='" + version + "']").load();
