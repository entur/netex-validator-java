...
public class InterchangeRuleReferencesIgnorer
  ...
  public Set<IdVersion> validateReferenceIds(
    ...
    return externalIdsToValidate
      .stream()
      .filter(
        InterchangeRuleReferencesIgnorer::isIgnorableReferenceFromInterchangeRule
      )
      .collect(Collectors.toUnmodifiableSet());
  }

  private static boolean isIgnorableReferenceFromInterchangeRule(
    IdVersion ref
  ) {
    return (
      "LineRef".equals(ref.getElementName()) &&
      ref.getParentElementNames().contains("InterchangeRule")
    );
  }
}
